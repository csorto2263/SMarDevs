...
export default function NewExamPage() {
  ...
  const [questions, setQuestions] = useState<Question[]>([
    { question_text: '', question_type: 'single_choice', points: 1, options: [
  ...
  const updateOption = (qi: number, oi: number, field: string, value: string | boolean) => {
    setQuestions(prev => prev.map((q, i) => {
      ...
      const newOptions = q.options.map((opt, j) => {
        if (j !== oi) {
          // For single_choice, uncheck others when setting one correct
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <h1 className="text-2xl font-bold text-navy-950">Create New Exam</h1>
      ...
      <div className="flex items-center justify-between bg-white rounded-xl border border-gray-100 p-4">
        ...
        <div className="flex gap-3">
          <Link href="/admin/exams" className="px-4 py-2 text-sm font-medium text-gray-600 hover:text-gray-800 transition-colors">
            Cancel
          </Link>
          <button
            onClick={handleSave}
